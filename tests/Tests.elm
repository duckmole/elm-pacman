              ...
              test "Update when press <- key " <|
                \() ->
                    let
                      (_, position) = (Update.pressesKey '\'' Model.init)
                    in
                      Expect.equal (2,1) position,
              test "Update when press <- key " <|
                \() ->
                    let
                      (_, position) = (Update.pressesKey '&' Model.init)
                    in
                      Expect.equal (1,2) position,
              test "Update when press <- key " <|
                \() ->
                    let
                      (_, position) = (Update.pressesKey '(' Model.init)
                    in
                      Expect.equal (1,0) position,
              test "Update when press 2x up key " <|
                \() ->
                    let
                      (_, position) = Update.pressesKey '(' (Update.pressesKey '(' Model.init)
                    in
                      Expect.equal (1,-1) position
            ]
        ]
